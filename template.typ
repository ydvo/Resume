// Customized version of simple-technical-resume by Himank Dave to be more like the Dartmouth College template
...
#let resume(
  ...
  show link: set text(fill: rgb("#1553db"))
  ...
  align(author-position, [
    #upper(text(author-font-size, weight: "extrabold")[#author-name])
    #v(-1em)
  ])
...
#let work-heading(company, location, title, start-date, end-date, body) = {
  // sanity checks
  assert.eq(type(start-date), datetime)
  assert(type(end-date) == datetime or type(end-date) == str)
...
// Pretty self-explanatory.
#let education-heading(institution, location, degree, degree2, gpa, start-date, end-date, body) = {
  // sanity checks
  assert.eq(type(start-date), datetime)
  assert(type(end-date) == datetime or type(end-date) == str)
